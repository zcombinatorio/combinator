...
async function getPoolFeeConfig(
  ...
  if (dao && dao.pool_type === 'dlmm') {
    ...
    if (dao.dao_pda.startsWith('PENDING') || dao.treasury_multisig?.startsWith('PENDING')) {
      console.log(`[DLMM Fee Claim] Skipping pending DAO ${dao.dao_name} - not yet finalized`);
      return null;
    }
    ...
    if (dao.dao_pda === PARTNER_DAO_PDA) {
      ...
      console.log(`[DLMM Fee Claim] Partner DAO ${dao.dao_name}: ${PARTNER_DAO_PROTOCOL_PERCENT.toFixed(2)}% protocol, ${PARTNER_DAO_TREASURY_PERCENT.toFixed(2)}% partner fee destination`);
    ...
    if (PARTNER_MANAGED_DAO_PDAS.has(dao.dao_pda)) {
      ...
      console.log(`[DLMM Fee Claim] Partner-managed DAO ${dao.dao_name}: ${PARTNER_MANAGED_PARTNER_PERCENT}% partner, ${PARTNER_MANAGED_DAO_PERCENT}% DAO, 0% protocol`);
    ...
    if (daoPercent > 0) {
      ...
      if (overrideWallet) {
        console.log(`[DLMM Fee Claim] DAO ${dao.dao_name}: using fee wallet override ${overrideWallet} instead of treasury ${dao.treasury_multisig}`);
      }
    }
