...
import {
  ...
  validateTreasuryGovernance,
  validateMintGovernance,
  type PoolInfo,
} from '../../lib/dao';
...
// ============================================================================
// POST /dao/parent-realms - Create a parent DAO with Realms governance
// ============================================================================

router.post('/parent-realms', requireSignedHash, async (req: Request, res: Response) => {
  try {
    const {
      wallet,
      name,
    ...
    // Validate required fields
    if (!name || !token_mint || !treasury_cosigner || !pool_address || !funding_signature || !treasury_governance || !mint_governance) {
      return res.status(400).json({
        error: 'Missing required fields: name, token_mint, treasury_cosigner, pool_address, funding_signature, treasury_governance, mint_governance'
      });
    }
    ...
    if (!isValidSolanaAddress(treasury_governance)) {
      return res.status(400).json({ error: 'Invalid treasury_governance address' });
    }
    if (!isValidSolanaAddress(mint_governance)) {
      return res.status(400).json({ error: 'Invalid mint_governance address' });
    }
    ...
    try {
      ...
      if (isToken2022) {
        return res.status(400).json({
          error: 'Token-2022 base mints are not currently supported',
          details: 'The Futarchy on-chain program requires SPL Token base mints. Token-2022 support is planned for a future release.',
          token_mint,
          token_program: 'Token-2022',
        });
      }
    ...
    // Validate Realms governance configuration BEFORE acquiring the lock
    const treasuryGovPubkey = new PublicKey(treasury_governance);
    const mintGovPubkey = new PublicKey(mint_governance);
    const cosignerPubkey = new PublicKey(treasury_cosigner);

    const treasuryValidation = await validateTreasuryGovernance(connection, treasuryGovPubkey, cosignerPubkey);
    if (!treasuryValidation.valid) {
      return res.status(400).json({
        error: 'Invalid treasury governance configuration',
        details: treasuryValidation.error,
      });
    }

    const mintValidation = await validateMintGovernance(connection, mintGovPubkey);
    if (!mintValidation.valid) {
      return res.status(400).json({
        error: 'Invalid mint governance configuration',
        details: mintValidation.error,
      });
    }
    ...
    try {
      ...
      const result = await client.initializeParentDAORealms(
        adminKeypair.publicKey,
        adminKeypair.publicKey,
        name,
        baseMint,
      ...
      // For Realms, the governance pubkeys ARE the authorities — no Squads vault derivation
      const treasuryAuthority = result.treasuryMultisig.toBase58();
      const mintAuthority = result.mintMultisig.toBase58();

      console.log(`[DAO] Created parent DAO ${name} (Realms governance)`);
      console.log(`  Treasury Governance: ${treasuryAuthority}`);
      console.log(`  Mint Governance:     ${mintAuthority}`);
      ...
      const dao = await createDao(pool, {
        ...
        pool_type,
        quote_mint,
        treasury_multisig: treasuryAuthority,
        mint_auth_multisig: mintAuthority,
        treasury_cosigner,
        dao_type: 'parent',
      ...
      console.log(`Created parent DAO (Realms): ${daoPda} (tx: ${tx}, funding: ${funding_signature})`);

      res.json({
        dao_pda: daoPda,
        moderator_pda: moderatorPda,
        treasury_governance: treasuryAuthority,
        mint_governance: mintAuthority,
        admin_wallet: adminWallet,
        pool_type,
        quote_mint,
        transaction: tx,
    ...
    console.error('Error creating parent DAO (Realms):', error);
    res.status(500).json({ error: 'Failed to create parent DAO', details: String(error) });
  }
});
